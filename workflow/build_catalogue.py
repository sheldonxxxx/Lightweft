...
def build(root):
    ...
    for a in assets:
        ...
        cards.append(f'''<article class="card" data-id="{e(a['id'])}" data-tags="{e('|'.join(a['tags']))}" data-search="{e(search)}" data-smoke="{str(bool(a.get('smoke_subset'))).lower()}" data-format="{e(original['format'])}">
...
<div class="body"><div class="identity"><h2>{e(a['id'])}</h2>{smoke}</div><p class="reason">{e(a['reason'])}</p><div class="tags">{chips}</div><p class="original-link"><a href="{e(original['path'])}" download>Original {original['format']} ↓</a><span>{e(details)}</span></p>
...
'''
    ...
    print(f'Built {n} catalogue cards; RAW smoke subset {smoke_ids}')
