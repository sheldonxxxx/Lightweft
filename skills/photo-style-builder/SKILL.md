...
Render through the actual editor. Retain a stable ID, parent version, intention, execution provenance, and editable state or recipe reference for each candidate. Publish candidates into the existing workspace review session instead of creating another helper website. If using Lightweft Review, locate its checkout and read the session and agent contract in `review/README.md` before preparing a manifest or consuming feedback. If this skill is installed without that application, use the user's available review surface and retain equivalent evidence in their workspace.
